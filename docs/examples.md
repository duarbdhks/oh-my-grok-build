...
Use only when ownership is truly disjoint. If two tasks must edit the same file, split waves or serialize with `/ogb-start`.

## 4. Independent re-verification of existing work
...
## 5. Long-running work with native `/goal`
...
## 6. Vague requirements → interview first
