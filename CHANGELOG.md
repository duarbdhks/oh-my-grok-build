...
- `N_ready` gained a fifth condition: a task that reads a file another selected task writes is a dependency, not a parallel peer. File-ownership disjointness alone let shared type modules, barrel exports, and generated interfaces slip into the same batch, which is exactly the class of conflict progressive integration no longer catches with a wave-wide comparison. The task card already carried `reads`; nothing consumed it before. `container` also joined the `iso_cap = 2` shared-resource list, which previously named it only in the command-overlap ban. `docs/architecture.md` and its Korean pair carry both corrections.
