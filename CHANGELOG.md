...
- `docs/architecture.md` and its Korean pair now describe dynamic concurrency (4 default, at most 8 with proven isolation, lowered on shared resources) instead of a flat default of 4. `docs/validation.md` and its pair gained a static scheduling-scenario mapping for `/ogb-ultrawork`, with live scenario runs and the shell-command background primitive recorded as still unverified.
