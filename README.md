...
[English](README.en.md) · [설계 평가](docs/upstream-evaluation.ko.md) · [아키텍처](docs/architecture.ko.md) · [로드맵](docs/roadmap.ko.md)
...
현재 버전은 `0.1.0`이며 **실험적 프리뷰**입니다. 구조와 정적 검증은 완료했지만, 릴리스 전 실제 Grok Build 세션에서 설치·호출·워크트리 병합 시나리오를 확인해야 합니다. 자세한 항목은 `docs/validation.ko.md`에 기록되어 있습니다.
